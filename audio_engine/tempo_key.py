...
ANALYSIS_SAMPLE_RATE = 22050  # downsample target for tempo/key only — cuts memory use
...
def _downsample_for_analysis(mono: np.ndarray, sample_rate: int):
    """Return (audio, sample_rate) downsampled to ANALYSIS_SAMPLE_RATE if the
    source is higher-resolution. Tempo/key detection doesn't need full
    resolution, and this meaningfully cuts memory use on constrained hosts."""
    mono32 = mono.astype(np.float32)
    if sample_rate > ANALYSIS_SAMPLE_RATE:
        y = librosa.resample(mono32, orig_sr=sample_rate, target_sr=ANALYSIS_SAMPLE_RATE)
        return y, ANALYSIS_SAMPLE_RATE
    return mono32, sample_rate
...
def estimate_tempo(mono: np.ndarray, sample_rate: int) -> dict:
    ...
    y_analysis, analysis_sr = _downsample_for_analysis(mono, sample_rate)
    ...
    if len(onset_env) > 10 and tempo_val > 0:
        ...
        period_frames = int(round((60.0 / tempo_val) * analysis_sr / 512))
